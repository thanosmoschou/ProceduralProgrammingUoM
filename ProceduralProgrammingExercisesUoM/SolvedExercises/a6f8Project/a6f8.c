...
int main()
{
	...
	maxpat1=Find_Max(3, N, Hosp1);
	maxpat2=Find_Max(2,N,Hosp2);

	sumhosp1=Find_Sum(3,N,Hosp1);
	sumhosp2=Find_Sum(2,N,Hosp2);

	Print_Data("GALINOS",3,N,Hosp1);
	Print_Data("ELPIS", 2,N,Hosp2);
	...
	res1=Find_Pat(3, N, Hosp1, patnam1, patsur1, &resu1);
	...
	res2=Find_Pat(2,N,Hosp2,patnam2,patsur2, &resu2);
...
void Print_Data(char name[30], int row, int D, Patient Hosp[])
{
	int i;
...
bool Find_Pat(int row, int D, Patient Hosp[], char name[30], char surn[30], int *ind)
{
	int i,ctr,j;
